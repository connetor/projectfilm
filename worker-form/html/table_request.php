<?php
	...
	require'../../class.php';
    $obj = new Dataphp();
    $sql ='SELECT `Request_ID`,`Member_ID`, `JobName`, `RequestDetail`, `IsConfirmRequest` FROM `request` WHERE `Worker_ID` = '.$_SESSION['id_show'];
   ...
   while ($data_ta = mysqli_fetch_assoc($res)) {
   		$sum++;
   	 	echo '<tr>
			    ...
			    <td>'.$obj->return_member($data_ta['Member_ID']).'</td>
